...
import { startOfMonth, startOfWeek, addWeeks, addMonths, addDays, format, subWeeks, subMonths } from 'date-fns'
...
export default function SchedulePage() {
  ...
  // Week-based navigation for admin roles
  const [weekStart, setWeekStart] = useState(() => startOfWeek(new Date(), { weekStartsOn: 1 }))
  // Month-based navigation for staff_hsc
  ...
  // Compute month filter for week view — fetch both months when week spans a boundary
  ...
  // Month filter for month view — single month
  const monthFilter = useMemo(() => format(monthStart, 'yyyy-MM'), [monthStart])

  const months = useMemo(() => isStaffHsc ? [monthFilter] : weekMonths, [isStaffHsc, monthFilter, weekMonths])
    ...
    <PageWrapper>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isStaffHsc ? (
            /* Month navigation for staff_hsc */
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setMonthStart((prev) => subMonths(prev, 1))}>
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="sm" onClick={handleToday} className="text-xs px-2">
                เดือนนี้
              </Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setMonthStart((prev) => addMonths(prev, 1))}>
                <ChevronRight className="h-4 w-4" />
              </Button>
              <span className="text-sm font-medium ml-1">
                {format(monthStart, 'MMMM yyyy', { locale: th })}
              </span>
            </div>
          ) : (
            /* Week navigation for admin roles */
            <div className="flex items-center gap-1">
        ...
        {isLoading ? (
          ...
          <ScheduleGrid
            data={schedules}
            weekStart={isStaffHsc ? monthStart : weekStart}
            onEdit={handleEdit}
          />
        )}
      </div>
